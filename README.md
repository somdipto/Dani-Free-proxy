...
- **App-bundled engine seed:** Set `DANI_FREE_ENGINE_SEED_BIN=<absolute path>` and `DANI_FREE_ENGINE_SEED_SHA256=<64-hex SHA-256 of that executable>` together. A release app bundles the exact executable for its OS/architecture, pins its hash and passes these values to the child. On the first start, the proxy copies it to `<DANI_FREE_HOME>/engine/bin/1.18.32/dani-engine[.exe]`, verifies the source and staged copy, and atomically promotes the copy. It does not run a user-visible installer or download the seed. A healthy previously installed copy is retained if the app seed becomes unavailable; a newer, already verified engine is also retained. A bad or missing seed on a clean installation fails the engine startup without a hidden network fallback (the app shows Dani Free unavailable/retrying). This copies a local engine, not the proxy itself: the proxy executable is inside the signed app resources. The packaged app should keep the key and engine state in its per-user data directory, never in read-only resources.
- **Remote engine updates:** Release binaries default to no GitHub release polling; set `DANI_FREE_ENGINE_AUTOUPDATE=1` only if explicitly opting into the existing GitHub update path. `DANI_FREE_ENGINE_AUTOUPDATE=0` always disables it. A scalable remote-update channel needs an independently hosted signed manifest and digest-addressed CDN artifacts; it is not implemented yet. App updates can replace the signed proxy and bundled engine seed without contacting GitHub at first launch. Model-list refresh remains independent of engine updates.
