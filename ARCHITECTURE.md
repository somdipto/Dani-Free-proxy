...
| HTTP 200 carrying an error envelope | provider | `{"error": …}` envelope (object, bare-string, string-array, list-of-error-objects, message-less numeric-code, bare-numeric, nested-envelope, plural-`errors`-key, Python-style `status_code`-key, or FastAPI-style `detail`-key, or nested-`detail`-key form, or `msg`-keyed form; a recognized rate-limit string nested under `msg` counts too) | fail over to the next model; an envelope 429 pauses like a real 429 |
