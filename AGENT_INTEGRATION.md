...
- Missing or unhealthy models are skipped. Transport errors, timeouts, 429 (pauses briefly, then advances to the next model), 5xx, and HTTP 200 with empty content fail over to the next model in the chain. A 4xx refusal is passed through to the caller as-is, with an `x-dani-free-model` header naming the model that answered.
